...
list_instance () {
   ...
   for i in ${instances[@]}; do
	   echo "$i $($awsi.Tags[].Value --instance-ids $i $ot) $($awsi.State[].Name --instance-ids $i $ot) $($awsi.Placement.AvailabilityZone --instance-ids $i $ot) $($awsi.PublicIpAddress --instance-ids $i $ot)"   
   done
   echo " "
}
